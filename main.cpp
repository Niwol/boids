...
class SimpleSimulationSystem : public Ra::Engine::Scene::System {
  ...
  const std::string colorHandleName =
      Ra::Engine::Data::AttribArrayDisplayable::getAttribName(
          Ra::Engine::Data::AttribArrayDisplayable::VERTEX_COLOR);

  Ra::Core::Utils::Attrib<Ra::Core::Vector4> &getColorAttrib() {
    auto cHandle =
        m_cloud->getCoreGeometry().getAttribHandle<Ra::Core::Vector4>(
            colorHandleName);
    return m_cloud->getCoreGeometry().getAttrib(cHandle);
  }

public:
  inline SimpleSimulationSystem(Ra::Engine::Data::PointCloud *c = nullptr)
      : m_cloud(c) {}
...
int main(int argc, char *argv[]) {
  ...
  //! [Colorize the cloud]
  cloud.addAttrib("in_color",
                  Ra::Core::Vector4Array{cloud.vertices().size(),
  ...
  auto c = new Ra::Engine::Scene::PointCloudComponent(
      "cloud Mesh", e, std::move(cloud), nullptr);
